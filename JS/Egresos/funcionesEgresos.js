...
//Cargar la tabla con todos los items del mes y anio actual
function cargarTablaCategEgr(categorias, cantidades, totales){
...
function obtenerCantidadesCategEgr(libroDM, categorias, mes, anio){
    //obtener totales de cada categoria
    ...
    categorias.forEach((categoria)=>{
        ...
        if (libroDM.mes === mes && libroDM.anio === anio){
            ...
            //calculo el total para esa categoria
            cantidad = egresos.filter((item)=> item.categoria === categoria).length;
